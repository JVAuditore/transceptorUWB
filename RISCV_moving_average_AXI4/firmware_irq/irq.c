...
uint32_t *irq(uint32_t *regs, uint32_t irqs)
{
    ...
    {
        mov_avg_result_irq = AXI_REG_READ(AXI_MOV_AVG_REG_OUT);	// Lê o resultado e limpa a interrupção no hardware

        ip_ready_flag = 1;	// Sinaliza ao main que o resultado está pronto
    }
